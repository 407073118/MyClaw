...
/**
 * 将 Vercel AI Gateway 模型目录响应归一化为统一模型目录项。
 */
export function normalizeVercelGatewayCatalog(
  payload: unknown,
  ...
  for (const item of rawItems) {
    ...
    normalized.push({
      ...
      provider,
      providerFlavor,
      contextWindowTokens: toNumber(item.context_window ?? item.context_length),
      maxOutputTokens: toNumber(item.max_tokens ?? item.max_output_tokens),
