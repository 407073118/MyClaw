...
import type { ChatMessage as ModelChatMessage, ResolvedToolCall } from "../services/model-client";
      ...
      try {
        while (round < SAFETY_CEILING) {
          ...
          if (hasToolCalls) {
            ...
            const executeSingleTool = async (toolCall: ResolvedToolCall): Promise<string> => {
              const toolCallId = toolCall.id;
              const toolId = functionNameToToolId(toolCall.name);
              const label = buildToolLabel(toolCall.name, toolCall.input);
            ...
            if (session.title === "New Chat" && session.messages.length >= 2) {
              ...
              if (userMsg) {
                const raw = userMsg.content.trim().split("\n")[0] ?? "";
