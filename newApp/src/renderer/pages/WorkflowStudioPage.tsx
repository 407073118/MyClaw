      ...
      <main className="studio-body">
        ...
        <section className="studio-canvas-area">
          ...
          {workflowDefinition && Array.isArray(workflowDefinition.nodes) ? (
            /* WorkflowCanvas would be mounted here — passed as a forwarded prop once the
               React component exists. For now, render a placeholder that preserves all
               event surface so tests and routing still work. */
            <div className="canvas-placeholder flex-fill">
              <p className="canvas-placeholder-text">
                画布加载中 — WorkflowCanvas 组件将在此渲染
              </p>
              <div className="node-list">
                {workflowDefinition.nodes.map((node: WorkflowNode) => (
                  <button
                    key={node.id}
                    type="button"
                    className={`node-chip${selectedNodeId === node.id ? " selected" : ""}`}
                    onClick={() => handleNodeSelection(node.id)}
                  >
                    <span className={`node-kind-dot node-kind-${node.kind}`} />
                    {node.label ?? node.id}
                  </button>
                ))}
              </div>
              {canvasFeedback && (
                <div className="canvas-feedback">{canvasFeedback}</div>
              )}
            </div>
          ) : definitionError ? (
          ...
          {showRunPanel && workflowDefinition && (
            <div className="run-panel-drawer">
              ...
              <div className="studio-run-panel">
                <p style={{ padding: "20px", color: "var(--text-secondary)", fontSize: 13 }}>
                  运行面板 — WorkflowRunPanel 组件将在此渲染
                </p>
              </div>
            </div>
          )}
              ...
              {workflowDefinition ? (
                <div className="inspector-placeholder">
                  <p style={{ color: "var(--text-secondary)", fontSize: 13 }}>
                    检查面板 — WorkflowGraphInspector 组件将在此渲染
                  </p>
                  {selectedNodeId && (
                    <p style={{ color: "var(--accent-cyan)", fontSize: 12, marginTop: 8 }}>
                      已选节点: {selectedNodeId}
                    </p>
                  )}
                  {selectedEdgeId && (
