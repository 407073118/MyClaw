...
const expectedStartLine = 439; // 0-indexed for 440
const expectedEndLine = 487; // 0-indexed for 488

if (lines[expectedStartLine].includes('.tab.active { ba')) {
    const newCss = `        .tab.active { background: var(--bg-card); color: var(--text-primary); box-shadow: 0 1px 2px rgba(0,0,0,0.2); }
    ...
    lines.splice(expectedStartLine, expectedEndLine - expectedStartLine + 1, ...newCss.split('\n'));
    fs.writeFileSync('src/renderer/pages/SettingsPage.tsx', lines.join('\n'));
    console.log("Replaced successfully!");
} else {
    console.log("Lines not matching expected format! Line 440 was:");
    console.log(lines[expectedStartLine]);
}
