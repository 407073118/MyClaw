...
function buildTaskPlanningContent(effort: string): string {
  ...
  const lines: string[] = [
    "You have task tools for decomposing and tracking work when the user explicitly wants a plan, when the task is long-running, or when progress must be visible.",
    "",
    "## When To Use Task Tools",
    "- Use task tools for explicit planning/tracking requests, complex multi-step implementation, delegated work, or long-running workflows.",
    "- Do not create tasks for ordinary quick Q&A, simple research, single-shot web lookup, or direct tool use; just do the work and answer.",
    "- If you create tasks, continue executing them in the same turn when safe. Stop only when you truly need the user's structured input.",
    "",
    "## Task Workflow",
    "1. **Create only useful tasks** — Call `task_create` for meaningful steps, not for every small answer.",
    "2. **Execute** — Work through tasks one by one: `task_update(id, status: \"in_progress\")` → do the work → `task_update(id, status: \"completed\")`.",
    "3. **Complete tracked tasks** — Do not leave tasks pending or in_progress unless `task_wait_for_user` has placed a task in waiting_user because the user must answer before work can continue.",
    "",
    "## Task Tools",
    ...
    "",
    "## Key Rules",
    "- **Avoid task noise** — A normal request like checking weather, answering a fact, or reading one file should not open a Task V2 chain.",
    "- **Track real work** — If you start a task chain, keep it accurate and finish every task you created.",
    "- **Discover new steps? Add tasks** — If substantial new work appears during execution, create a task for it.",
    ...
    "- **Skip tasks for**: direct factual Q&A, greetings, simple research, ordinary web lookup, or clarification questions.",
  ];
  if (effort === "high") {
    lines.push(
      "",
