...
export type TimeJobExecutorDeps = {
  ...
  runAssistantPrompt: (input: { job: ScheduleJob; prompt: string }) => Promise<{ outputSummary: string; sessionId: string }>;
};
    ...
    async execute(job: ScheduleJob): Promise<TimeJobExecutionResult | void> {
      ...
      switch (job.executor) {
        ...
        case "assistant_prompt": {
          ...
          const { outputSummary, sessionId } = await deps.runAssistantPrompt({ job, prompt });
          ...
          return { outputSummary: truncated, sessionId };
        }
